...
check-healthcare-gql: _prepare-gerbil-parser
    cd "{{ contribution_source_root }}/lambda-episteme" && GERBIL_PATH="{{ gerbil_parser_path }}" GERBIL_LOADPATH="{{ contribution_test_library_path }}:{{ gerbil_parser_library_path }}" gerbil interactive -e '(begin (import (only-in :clan/poo/object .ref) (only-in :std/misc/ports read-all-as-string) (only-in :poo-flow/src/modules/query/objects poo-flow-gql-query-program) (only-in :poo-flow/lambda-episteme/user-interface/scenarios/healthcare/assurance healthcare-case-qualification-gql-paths) (only-in :gerbil-parser/languages/gql/iso-39075-2024/parser +gql-syntax-contract+ parse-gql-iso-39075-2024) (only-in :gerbil-parser/src/runtime/artifact parse-artifact-ref parse-artifact-success? parse-artifact-valid? parse-artifact-roundtrip)) (let loop ((paths (healthcare-case-qualification-gql-paths)) (receipts (quote ()))) (if (null? paths) (begin (for-each displayln (reverse receipts)) (exit 0)) (let* ((path (car paths)) (source (call-with-input-file path read-all-as-string)) (artifact (parse-gql-iso-39075-2024 source)) (program (poo-flow-gql-query-program path path (parse-artifact-ref artifact (quote sourceDigest)) (parse-artifact-ref artifact (quote grammarDigest)) +gql-syntax-contract+)) (accepted? (and (parse-artifact-success? artifact) (parse-artifact-valid? artifact) (equal? source (parse-artifact-roundtrip artifact)) (eq? (.ref program (quote parser-identity)) (quote gerbil-parser)) (equal? (.ref program (quote syntax-contract)) +gql-syntax-contract+)))) (unless accepted? (displayln (list (cons (quote source) path) (cons (quote accepted) #f))) (exit 1)) (loop (cdr paths) (cons (list (cons (quote source) path) (cons (quote parser) (quote gerbil-parser)) (cons (quote syntax-contract) +gql-syntax-contract+) (cons (quote source-content-id) (.ref program (quote source-content-id))) (cons (quote grammar-content-id) (.ref program (quote grammar-content-id))) (cons (quote accepted) #t) (cons (quote roundtrip) #t)) receipts))))))'
