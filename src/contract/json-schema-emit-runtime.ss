...
;; Native Contract admission accepts either a POO object or JSON object rows.
;; The emitted Contract owns this bounded representation boundary; consumers do
;; not recover an executable predicate from a projected record.
(def (poo-flow-json-schema-contract-candidate-entry candidate slot)
  (cond
   ((object? candidate)
    (and (.slot? candidate slot) (cons slot (.ref candidate slot))))
   ((poo-flow-contract-json-object? candidate)
    (poo-flow-json-schema-object-entry candidate slot))
   (else #f)))

(def (poo-flow-json-schema-contract-slot-present? candidate slot)
  (if (poo-flow-json-schema-contract-candidate-entry candidate slot) #t #f))

(def (poo-flow-json-schema-contract-slot-ref candidate slot)
  (let (entry (poo-flow-json-schema-contract-candidate-entry candidate slot))
    (if entry (cdr entry) #f)))

(def (poo-flow-json-schema-contract-candidate? candidate)
  (or (object? candidate)
      (poo-flow-contract-json-object? candidate)))
