    ...
    (test-case "validates effective policy and reports denied session edges"
      (let* ((context (make-session-policy-validation-context))
        ...
        (check-equal?
         ...
         #f)
        (check-equal? (.ref receipt 'effective-model-ref)
                      'marlin/model/build-review)
        (check-equal? (.ref receipt 'effective-prompt-session-ref)
                      'session/build-system)
        (check-equal? (.ref receipt 'allowed-context-refs)
                      '(session/root))
        (check-equal? (.ref receipt 'denied-context-refs)
                      '(session/audit))
        (check-equal? (.ref receipt 'allowed-communication-channels)
                      '(channel/build-root))
        (check-equal? (.ref receipt 'denied-resource-refs)
                      '(network-egress))
        (check-equal? (length (.ref receipt 'allowed-agent-tool-attempts))
                      2)
        (check-equal? (length (.ref receipt 'denied-agent-tool-attempts))
                      1)
        (check-equal? (length (.ref receipt 'allowed-hook-tool-attempts))
                      1)
        (check-equal? (length (.ref receipt 'denied-hook-tool-attempts))
                      1)
