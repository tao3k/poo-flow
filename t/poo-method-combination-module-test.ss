...
(import (only-in :std/test test-suite test-case check-equal?)
        ...
        (only-in :poo-flow/src/module-system/poo-method-combination/config
        ...
        (only-in :poo-flow/src/modules/funflow/interface
...
(def poo-method-combination-module-test
  (test-suite "poo-method-combination first-class module boundary"
    (test-case "compatibility declaration macros retain executable witnesses"
      (let ((direct
             (poo-flow-module-bundles
              (flow funflow +dag)))
            (custom
             (poo-flow-custom-module-bundles
              (local-module "./modules/local-module" +doctor)))
            (init
             (poo-flow-init-module-bundles
              :workflow (funflow +dag)
              :sandbox (nono-sandbox +doctor))))
        (check-equal?
         (poo-flow-user-module-selection-key (caar direct))
         '(flow . funflow))
        (check-equal?
         (poo-flow-user-module-selection-key (caar custom))
         '(custom . local-module))
        (check-equal?
         (map (lambda (bundle)
                (poo-flow-user-module-selection-key (car bundle)))
              init)
         '((flow . funflow) (sandbox . nono-sandbox)))))
      ...
      (let ((keys
            ...
            (downstream
             (poo-flow-modules!
              ...
              :flow (funflow +dag))))
        (check-equal? (length keys) 17)
        (check-equal?
         (and (not (member (cons 'core 'poo-method-combination) keys))
        ...
        (check-equal?
         ...
         poo-flow-maintained-module-bundles)
         #t)))
    (test-case "module-system implementation features own modular directories"
      ...
      (check-equal?
       (all-files-exist?
        '("src/module-system/poo-method-combination/types.ss"
          ...
          "src/module-system/observability/interface.ss"
          "src/modules/workflow/store.ss"))
       #t)
      (check-equal? (file-exists? "src/observability") #f)
      (check-equal? (file-exists? "src/workflow/store.ss") #f))
    (test-case "registry derives the canonical config entrypoint"
      (let (paths
            (map poo-flow-module-source-ref-value
                 (poo-flow-src-modules-source-refs)))
        (check-equal?
         (and (member "src/module-system/poo-method-combination/config.ss"
                      paths)
              #t)
         #t)))
