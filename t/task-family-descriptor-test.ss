...
(def task-family-descriptor-test
  (test-suite "task family descriptors"
    ...
    (test-case "routes tasks through descriptor policy"
      (let ((pure (make-pure-task 'inc (lambda (x) (+ x 1)) 'number 'number))
        ...
        (check-equal? (task-capability put) 'store)
        ...
        (check-equal? (task-adapter-operation put) 'store-put)
        (check-equal? (task-adapter-operation get) 'store-get)
